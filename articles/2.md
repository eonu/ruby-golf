...
All of these examples either iterate through the array and call a method on each individual element, e.g. `x.length`, `x.reverse`, or call a method with the element as a parameter e.g. `print x`, `Math.sqrt(x)` and `File.read(f)`. They can all be shortened by using the `&` operator. If you are calling a method on the elements of the array, it's straightforward:
...
As you can see though, in a lot of these kinds of cases, it's almost always better to just stick with the literal `{}` block format, due to `.method` costing quite a few bytes, and having to use parentheses:
...
But as you can see, these situations are extremely rare, and in this case only 3 bytes were saved.
